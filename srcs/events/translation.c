...
void	translate_camera(int key, t_data *data)
{
    ...
    move_amount = 0.5;
    if (key == XK_a)
        data->camera->coordinates = vec_sub(
            data->camera->coordinates,
            vec_scalar_mult(data->camera->right, move_amount));
    if (key == XK_d)
        data->camera->coordinates = vec_add(
            data->camera->coordinates,
            vec_scalar_mult(data->camera->right, move_amount));
    if (key == XK_w)
        data->camera->coordinates = vec_add(
            data->camera->coordinates,
            vec_scalar_mult(data->camera->normalized, move_amount));
    if (key == XK_s)
        data->camera->coordinates = vec_sub(
            data->camera->coordinates,
            vec_scalar_mult(data->camera->normalized, move_amount));
